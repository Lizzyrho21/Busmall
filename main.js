...
class BusMallPictures { //This is a class constructor function we will be the base of the ENTIRE project
    ...
    timesShown = 0; // amounst of times for said image
        ...
        // Initial constructor (MUST BE NAME 'CONSTRUCTOR')
         constructor (nameofPicture, imageSrc) // another method
        {
...
//  A: We want to connect the image ids with the images and the text ids with the text.
 ...
 // ======================= CREATE A FUNCTION THAT WILL RANDOMLY GENERATE OUR PICTURES ======================== //
    ...
    // Update Middle busmall picutres to show up in DOM. the random photo is generated inside of the square brackets 
     ...
     MIDDLE_BUSMALL_TEXT.innerText = allBusmallPics[randomPhotoMiddle].nameofPicture;
...
const handleClickOnPicture = function (evt) { // a function is stored inside of a variable we use to handle the picture clicked
    console.log(`You clicked this target element id ${evt.target.id}`);
    // if they can still click, do clicky things
    if (totalClicks < MAX_CLICKS) { // condition to check if the total clicks the user generates is less than the amount WE SPECIFY.
        ...
        if (id === 'left-busmall-img' || id === 'right-busmall-img') {
            ...
            if (id === 'middle-busmall-img') { // clicked on the middle image
                ...
                console.log(`Middle pic ${middleBusMallPic.nameofPicture} has ${middleBusMallPic.clicks} so far`);
            }
